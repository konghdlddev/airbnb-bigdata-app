...
st.title("Geo-Aware Recommendations")
st.caption("Find similar listings based on zone, room type, transit accessibility, tourist profile, and price behavior.")

listing_ids = get_listing_id_options()
if not listing_ids:
    st.warning("No listing IDs found in Gold dataset. Run ETL first.")
    st.stop()

with st.form("recommendations_form"):
    listing_id = st.selectbox("Source Listing ID", options=listing_ids, index=0)
...
if submitted:
    ...
    if recommendations is None or recommendations.empty:
        st.warning("No recommendations available for this listing.")
    else:
